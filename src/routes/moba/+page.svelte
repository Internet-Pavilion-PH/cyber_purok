<script lang="ts">
  ...
  const resetGameCameraToHero = () => {
    ...
    gameCamera.alpha = defaultGameCamera.alpha;
    gameCamera.beta = defaultGameCamera.beta;
    gameCamera.radius = defaultGameCamera.radius;
    gameCamera.target = heroMesh.position.clone();
  };
  ...
  onMount(() => {
    ...
    const createScene = () => {
      ...
      gameCamera = new BABYLON.ArcRotateCamera(
        "gameCamera",
        defaultGameCamera.alpha,
        defaultGameCamera.beta,
        defaultGameCamera.radius,
        heroStartPosition,
        scene,
      );
      scene.activeCamera = gameCamera;

      gameCamera.attachControl(canvas, false);
      const mouseInput = gameCamera.inputs.attached.mouse as any;
      const pointerInput = gameCamera.inputs.attached.pointers as any;
      if (mouseInput) mouseInput.buttons = [];
      if (pointerInput) pointerInput.buttons = [];
